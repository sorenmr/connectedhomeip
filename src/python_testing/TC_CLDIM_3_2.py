...
class TC_CLDIM_3_2(MatterBaseTest):
    ...
    def steps_TC_CLDIM_3_2(self) -> list[TestStep]:
        steps = [
            ...
            TestStep("5b", "Send Step command"),
            TestStep("5c", "Send Step command"),            
            TestStep("5d", "Read TargetPositioning attribute"),
            TestStep("5e", "Wait for motion"),
            TestStep("5f", "Read CurrentPositioning attribute"),
         ]
        return steps
    ...
    async def test_TC_CLDIM_3_2(self):
        ...
        #STEP 5b: Send Step command
        self.step("5b")
        await self.send_single_cmd(cmd=Clusters.Objects.ClosureDimension.Commands.Step(Direction=StepDirectionEnum.TowardsMin, NumberOfSteps=1), endpoint=self.endpoint)

        #STEP 5c: Send Step command
        self.step("5c")
        await self.send_single_cmd(cmd=Clusters.Objects.ClosureDimension.Commands.Step(Direction=StepDirectionEnum.TowardsMin, NumberOfSteps=1), endpoint=self.endpoint)

        #STEP 5d: Read TargetPositioning attribute
        self.step("5d")
        ...
        #Validate values
        expectedPosition = max(MinPosition, MaxPosition - 3*StepValue)
        asserts.assert_equal(targetPositioning.Position, expectedPosition, "Unexpected value")

        #STEP 5e: Wait for motion
        self.step("5e")
        wait_for_motion(self, StepMotionDuration*3)

        #STEP 5f: Read CurrentPositioning attribute
        self.step("5f")
        currentPositioning = await self.read_single_attribute_check_success(endpoint=self.endpoint, cluster=cluster, attribute=attributes.CurrentPositioning)
